...
const ServiceDetailCard = ({ service }: ServiceDetailCardProps) => {
  ...
  const historyItems = [
    ...
    ...(!sameDate ? [{ label: 'Ultima atualizacao', value: formatDateTime(service.updated_at) }] : []),
  ...
  const title = service.service_type_name || service.service_type_key || 'Servico operacional'
    ...
    <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Situacao operacional</p>
          <p className="mt-1 text-sm font-semibold text-slate-800">{label(OPERATIONAL_LABELS, service.operational_status)}</p>
          <p className="mt-2 text-xs text-slate-600">Inicio: {formatDateTime(service.start_at)}</p>
          <p className="text-xs text-slate-600">Duracao: {service.duration_hours}h</p>
        </div>

        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Situacao financeira</p>
      ...
      {service.notes ? (
        <div className="rounded-xl border border-slate-200 p-3">
          <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Observacoes</p>
